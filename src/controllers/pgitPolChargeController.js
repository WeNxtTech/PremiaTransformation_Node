const pgitPolChargeService = require('../services/pgitPolChargeService');
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    const result = await pgitPolChargeService.getAll(filters, queryOptions);
...
exports.create = async (req, res, next) => {
  try {
    const result = await pgitPolChargeService.create(req.body);
...
exports.update = async (req, res, next) => {
  try {
    const result = await pgitPolChargeService.update(req.params.id, req.body);
...
exports.deleteItem = async (req, res, next) => {
  try {
    const result = await pgitPolChargeService.deleteItem(req.params.id);
...
exports.getByPolSysId = async (req, res, next) => {
  try {
    const { pchgPolSysId } = req.query;

    if (!pchgPolSysId) {
      return res.status(400).json({
        success: false,
        message: "pchgPolSysId is required"
      });
    }

    const result = await pgitPolChargeService.getByPolSysId(Number(pchgPolSysId));
