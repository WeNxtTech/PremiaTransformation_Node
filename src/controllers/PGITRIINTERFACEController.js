...
exports.getByPolSysId = async (req, res, next) => {
  try {
    const { RI_POL_SYS_ID } = req.query;

    if (!RI_POL_SYS_ID) {
      return errorResponse(res, 400, "RI_POL_SYS_ID is required");
    }
    ...
    const result = await PGITRIINTERFACEService.getByPolSysId(polSysIdNum);
